...
- (void)setUp
{
    ...
    {
        [self setContentOffset: CGSizeMake(0, -450.0)];
        [self setViewpointOffset:CGSizeMake(0, -575.0)];
    }
    else
    {
        [self setContentOffset: CGSizeMake(0, -486.0)];
        [self setViewpointOffset:CGSizeMake(0, -600.0)];
    }
}
    ...
    switch (_type)
    {
        ...
        case iCarouselTypeInvertedRotary:
        {
            if([[UIDevice currentDevice]userInterfaceIdiom]==UIUserInterfaceIdiomPhone)
            {
                ...
                CGFloat spacing = [self valueForOption:iCarouselOptionSpacing withDefault:1.6f];
            ...
            {
                CGFloat count = [self circularCarouselItemCount];
                CGFloat spacing = [self valueForOption:iCarouselOptionSpacing withDefault:2.1f];
                CGFloat arc = [self valueForOption:iCarouselOptionArc withDefault:M_PI * 2.0f];
                CGFloat radius = [self valueForOption:iCarouselOptionRadius withDefault:fmaxf(_itemWidth * spacing / 2.0f, _itemWidth * spacing / 2.0f / tanf(arc/2.0f/count))];
                CGFloat angle = [self valueForOption:iCarouselOptionAngle withDefault:offset / count * arc];
...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    //center view
    view.superview.center = CGPointMake(self.bounds.size.width/2.0f + _contentOffset.width,self.bounds.size.height/2.0f + _contentOffset.height);
